...
def parse_args_httpfaker():
    ...
    parser.add_argument('--project-name', nargs='?', action='store', default='httpfaker-project',
                        help='项目名， 默认httpfaker-project')
    ...
    parser.add_argument('--headers', nargs='?', action='store', default=None, help='配置访问proxy服务器时用到的header')
    args = parser.parse_args()
    if args.headers:
        headers = {}
        for x in args.headers.split(';'):
            k, v = x.split('=')
            headers[k] = v
...
def parse_args_swagger2api():
    ...
    parser.add_argument('--headers', nargs='?', action='store',
                        help='新增header，多个使用;分割。格式：Content-Type=application/json;Authorization=xxxxxx')
    args = parser.parse_args()
    if args.headers:
        headers = {}
        for x in args.headers.split(';'):
